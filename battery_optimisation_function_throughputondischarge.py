...
def battery_optimisation(datetime, spot_price, asset_params, initial_capacity=0, initial_SoH = 2, include_revenue=True, solver: str='glpk'):
    ...
    MAX_DAILY_THROUGHPUT = MAX_BATTERY_CAPACITY * EFFICIENCY * asset_params['DAILY_HARD_CAP'] # redefine for degraded throughput
    MAX_YEARLY_THROUGHPUT = MAX_BATTERY_CAPACITY * EFFICIENCY * asset_params['SOFT_CAP'] * 365 # redefine for degraded throughput and apply correctly
    ...
    def soh_constraint(battery, i):
        if i == battery.Period.first():
            ...
            return battery.SoH[i] == battery.SoH[i-1] - (((battery.Discharge_power[i-1] * EFFICIENCY) / 2) * DEG_FACTOR)
    ...
    def over_charge(battery, i):
        return battery.Charge_power[i] <= (battery.SoH[i] - battery.Capacity[i]) * 2 / sqrt(EFFICIENCY)
    ...
    def capacity_constraint(battery, i):
        ...
        return battery.Capacity[i] == (battery.Capacity[i-1] - SELF_DISCHARGE_RATE
                                        + (battery.Charge_power[i-1] * sqrt(EFFICIENCY) / 2) 
                                        - (battery.Discharge_power[i-1] * sqrt(EFFICIENCY) / 2))
    
    #Defining throughput constraints
    
    def daily_throughput_accumulation_constraint(battery, i):
        if i == battery.Period.first():
            return battery.throughput_daily[i] == (battery.Discharge_power[i]  * EFFICIENCY) / 2
        if (i-1) % 48 == 0:
            return battery.throughput_daily[i] == (battery.Discharge_power[i]  * EFFICIENCY) / 2
        else:
            return battery.throughput_daily[i] == battery.throughput_daily[i - 1] + (battery.Discharge_power[i]  * EFFICIENCY)  / 2

    def yearly_throughput_accumulation_constraint(battery, i): 
        if i == battery.Period.first():
            return battery.throughput_yearly[i] == (battery.Discharge_power[i]  * EFFICIENCY) / 2
        else:
            if (i-1) % 17520 == 0:
                return battery.throughput_yearly[i] == battery.throughput_yearly[i - 1] + (battery.Discharge_power[i]  * EFFICIENCY)  / 2
            else: 
                return battery.throughput_yearly[i] == battery.throughput_yearly[i - 1] + (battery.Discharge_power[i]  * EFFICIENCY)  / 2
    ...
    result['throughput'] = (result['discharge_power'] * EFFICIENCY) / 2

    result['cycles'] = result['throughput'] / result['SoH']

    final_capacity = result['opening_capacity'].iloc[-1] - SELF_DISCHARGE_RATE + ((result['charge_power'].iloc[-1] * sqrt(EFFICIENCY)) / 2) - ((result['discharge_power'].iloc[-1] * sqrt(EFFICIENCY)) / 2 )

    SoH = result['SoH'].iloc[-1] - (((result['discharge_power'].iloc[-1] * EFFICIENCY)) / 2 ) * DEG_FACTOR
